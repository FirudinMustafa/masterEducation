...
const NAV_ITEMS = [
  ...
  },
  {
    href: "/admin/bayiler",
    label: "Bayiler",
    countKey: "pendingDealers" as const,
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
...
/**
 * Inner nav body — both desktop sidebar and mobile drawer share this content.
 * `onNavigate` close callback'i sayesinde drawer link tiklaninca otomatik kapanır.
 */
type NavCounts = { pendingOrders: number; pendingDealers: number };
...
function AdminNavBody({ onNavigate }: { onNavigate?: () => void }) {
  ...
  // Bekleyen sipariş/başvuru sayıları — route değişiminde yeniden çekilir
  // (admin bir siparişi onaylayınca/durumu değişince rozet güncellenir).
  useEffect(() => {
    let cancelled = false;
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="flex items-center gap-2 px-3 py-2 mb-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-500 uppercase tracking-wider">Master Education</p>
          </div>
        </div>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ? "bg-brand-gold-light/50 text-brand-black"
              ...
              >
                ...
                {badge > 0 && (
                  <span className="inline-flex min-w-[1.25rem] items-center justify-center rounded-full bg-rose-500 px-1.5 py-0.5 text-[11px] font-bold leading-none text-white">
                    {badge > 99 ? "99+" : badge}
                  </span>
                )}
              </Link>
      ...
      <div className="shrink-0 border-t border-gray-100 p-4 bg-white">
        {session?.user && (
          <div className="mb-3 flex items-center gap-2.5 px-1">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-gold-light text-sm font-bold text-brand-black">
              {(session.user.name ?? session.user.email ?? "?").charAt(0).toUpperCase()}
            </span>
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-semibold text-brand-black">
                {session.user.name ?? "Yonetici"}
              </p>
        ...
        <button
          onClick={() => signOutWithCleanup("/yonetim")}
          className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-rose-600 hover:bg-rose-50 transition-colors cursor-pointer"
        >
...
export function AdminSidebar() {
  return (
    <aside className="w-60 bg-white border-r border-gray-200 shrink-0 hidden md:flex md:flex-col md:h-screen md:sticky md:top-0">
      <AdminNavBody />
    </aside>
  );
}
...
/**
 * Mobile drawer — AdminMobileHeader içindeki ☰ butonuna bağlı. Tüm nav item'leri
 * drawer'da, link tıklanınca otomatik kapanır + body scroll-lock.
 */
    ...
    <div className="fixed inset-0 z-[60] bg-black/40 md:hidden" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between border-b border-gray-100 p-3">
          <span className="text-sm font-bold text-brand-black">Yonetim Menu</span>
          <button
            onClick={onClose}
            aria-label="Kapat"
        ...
        <AdminNavBody onNavigate={onClose} />
      </aside>
    </div>
  );
}
...
/**
 * Mobile menu trigger — AdminMobileHeader içine konur.
 */
export function AdminMobileMenuButton({ onOpen }: { onOpen: () => void }) {
  return (
    <button
      ...
      className="flex h-9 w-9 items-center justify-center rounded-lg text-gray-700 hover:bg-gray-100 md:hidden cursor-pointer"
    >
...
/**
 * State container — drawer + button + state'i tek client comp'ta tutar; mobile
 * header bunu render eder.
 */
export function AdminMobileNavTrigger() {
  const [open, setOpen] = useState(false);
  return (
    <>
      <AdminMobileMenuButton onOpen={() => setOpen(true)} />
