...
import { AdminMobileNavTrigger } from "./sidebar";

/**
 * Mobile-only top bar — ☰ menu drawer trigger + Admin label + Siteye Git link.
 * Çıkış + email drawer içine taşındı (drawer footer'da görünür) — buradaki
 * sıkışıklık giderildi.
 */
    ...
    <div className="md:hidden sticky top-0 z-30 bg-white border-b border-gray-200 px-3 py-2 flex items-center justify-between gap-2">
      <div className="flex items-center gap-2 min-w-0">
        ...
        <Link href="/admin" className="flex items-center gap-2 min-w-0">
          <div className="w-7 h-7 rounded-lg bg-brand-gold flex items-center justify-center shrink-0">
            <svg className="w-3.5 h-3.5 text-brand-black" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
          ...
          <span className="font-semibold text-sm text-brand-black truncate">Admin</span>
        </Link>
      </div>
      <Link
        href="/"
